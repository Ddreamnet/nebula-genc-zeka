...
/**
 * `fullBleed` marks the letter badges: they carry their own disc and must not
 * be inset inside the white pill, or the letters end up rendering in a circle
 * a third smaller than the real logos beside them.
 */
type Mark = ((props: { className?: string }) => ReactElement) & { fullBleed?: boolean };
...
/**
 * Plain colored-letter badge for brands with no verified mark available (see
 * file header).
 *
 * Drawn as SVG in the same 24×24 box the real marks use, so the letters scale
 * with the badge instead of sitting at a fixed 11px: the old HTML version
 * overflowed its own circle at 32px and was clipped to a single letter at the
 * 26px the bar chip uses ("xAI" came out as a sliced "A"). `textLength` pins
 * the widest labels to the disc, so a three-letter brand cannot spill again.
 */
function monogram(label: string, bg: string) {
  const fontSize = label.length >= 3 ? 9 : label.length === 2 ? 11.5 : 14;
  const Monogram: Mark = function Monogram({ className }: { className?: string }) {
    return (
      <svg viewBox="0 0 24 24" className={className} aria-hidden>
        ...
        <text
          ...
          fontWeight={700}
          letterSpacing={-0.3}
          textLength={label.length >= 3 ? 17 : undefined}
  ...
  Monogram.fullBleed = true;
  return Monogram;
}

// No verified OpenAI mark exists (removed from simple-icons at the company's
// own request), so it gets the same letter badge as the others. It used to be
// a clock face, which is not a logo of anything.
const OpenAIMonogram = monogram("OA", "#0f0f0f");
...
export function ProviderBadge({
  ...
  // The inset is a share of the badge, not a fixed 6px: this is drawn at 26px
  // in the bar chip and at 48px on the landing page, and one fixed padding
  // left the mark at half the badge on the small one and swimming in white on
  // the large one. It is expressed as the MARK's own size rather than as
  // padding on purpose — a percentage padding resolves against the parent's
  // width, not the badge's, which collapses the mark to nothing.
  return (
    <span
      className={cn(
        "inline-flex shrink-0 items-center justify-center overflow-hidden rounded-full",
        Mark.fullBleed ? "bg-transparent" : "bg-white shadow-sm",
        className,
      )}
      style={style}
    >
      <Mark className={Mark.fullBleed ? "size-full" : "size-[70%]"} />
    </span>
  );
}
